...
def streaming_chatbot() -> None:
    ...
    while True:
        ...
        # Add assistant reply vào history
        history.append({"role": "assistant", "content": assistant_reply})

        # Giữ lại 3 turns gần nhất (1 turn = user + assistant)
        # 3 turns = 6 messages
        history = history[-6:]
...
def batch_compare(prompts: list[str]) -> list[dict]:
    ...
    for prompt in prompts:
        ...
        result["prompt"] = prompt   # <-- thiếu dòng này
...
def format_comparison_table(results: list[dict]) -> str:
    ...
    for result in results:
        table += f"{result['prompt']} | {result['gpt4o_response'][:40]} | {result['mini_response'][:40]} | {result['gpt4o_latency']} | {result['mini_latency']}\n"
    return table
